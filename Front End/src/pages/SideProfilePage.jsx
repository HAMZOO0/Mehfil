...
import { UserProfileHeader, Header, Layout } from "../components/index.js";
const Sidepage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setIsFollow(userData?.isFollow || false);
  ...
  return (
    <Layout>
      <Header />
      <UserProfileHeader />
      <aside className="block md:block lg:col-span-3 bg-gray-900 sticky top-[100px] border p-4 min-w-48 text-white max-h-full">
        <img
          className="mb-3 flex aspect-square h-20 w-20 flex-shrink-0 rounded-full object-cover"
          src={avatar}
          alt="avatar"
        />
        <h2 className="text-3xl mb-1">{user_name}</h2>
        <p className="text-sm">
          <button className="hover:text-[#ae7aff] text-[#ae7aff] text-sm mb-2 py-3">
            {Email}
          </button>
        </p>
        <hr className="my-3 h-[1px] w-full bg-gray-600" />
